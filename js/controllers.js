...
//        }
        ...
        .controller('DoctorConsultationsPatientActiveCtrl', function ($scope, $http, $ionicModal, $stateParams, $filter, $ionicPopup, $timeout, $ionicHistory, $filter, $state, $ionicFilterBar, $ionicLoading) {
            $ionicLoading.show({template: 'Loading..'});
            $scope.drId = get('id');
            $scope.userId = get('id');
            $scope.interface = get('interface_id');
            ...
            $scope.doRefresh = function () {
                ...
                $http({
                    method: 'GET',
                    url: domain + 'appointment/get-patient-active-details',
                    params: {id: $scope.drId,patientid : get('patientId')}
                }).then(function successCallback(response) {
                    console.log(response.data);
                    $scope.todays_data = response.data.todays_data;
                    $scope.todays_app = response.data.todays_appointments;
        ...
        .controller('DoctorConsultationsPatientPastCtrl', function ($scope, $http, $stateParams, $filter, $ionicPopup, $timeout, $ionicHistory, $filter, $state, $ionicFilterBar, $ionicLoading) {
            $ionicLoading.show({template: 'Loading..'});
            unset(['backurl', 'recId', 'appId']);
            $scope.drId = get('id');
            $scope.userId = get('id');
